...
void setup() {
  #if defined(DISCO_BUZZER) && defined(PROMICRO)
  pinMode(7, OUTPUT);
  #endif
  #if !defined(GPS_PROMINI)
...
void loop () {
 ...
 //*********************************** 
  ...
  writeMotors();
  #if defined(DISCO_BUZZER) && defined(PROMICRO)
    #if defined(DISCO_CHANNEL)
      if (rcData[DISCO_CHANNEL] > 1500)
    #elif defined(FAILSAFE)
      if (failsafeCnt > (5*FAILSAFE_DELAY))
    #else
      #error Must define DISCO_CHANNEL or FAILSAFE with DISCO_BUZZER
    #endif
