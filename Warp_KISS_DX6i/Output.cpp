...
void writeMotors() { // [1000;2000] => [125;250]
  ...
  /******** Specific PWM Timers & Registers for the atmega32u4 (Promicro)   ************/
    ...
    Temp2 = motor[3] - 1000;
      ...
      #elif defined(ONESHOT)
        {
          uint16_t m3 = 2047 - motor[3];
          TC4H = m3 >> 8; OCR4D = (m3 & 0xFF); //  pin 6
        }
      #else
    ...
    #endif
    #if defined(ONESHOT)
      TCNT1 = 0xFFFF;  OCR1A=0; OCR1B=0;  // start oneshots timer 1
      TCNT3 = 0xFFFF;  OCR3A=0;           // start oneshots timer 3
      TCCR4B = B00000011;                 // Start Timer4 Prescaler = 4
    #endif
  #endif
...
void initOutput() {
  ...
  #if defined(MOTOWII_REVB)
    DDRE &= ~(1<<2);                  // Port E2 is input
    PORTE |= (1<<2);                  // Enable pull-up resistor
  #endif
  ...
  /******** Specific PWM Timers & Registers for the atmega32u4 (Promicro)   ************/
        ...
        ICR3   |= 0xFFFF; // TOP to 16-bit;    
        TCCR3A |= _BV(COM3A1); // connect pin 5 to timer 3 channel A    
        OCR3A = 0;
      #endif
      #if (NUMBER_MOTOR >3)
        // INIT Timer4 for HW-Oneshot with Compare Unit OC4D
	      // OPERATION: enhanced PWM Mode, Fast PWM,
        // only Pins OC4x connected with inverted OCnx signals
        TCNT4 = 0; TCCR4B = 0; TCCR4D = 0; TCCR4E = 0; TIMSK4 = 0; TCCR4A = 0; TCCR4C = 0;  
        // set Waveform Generation Mode Bits PWM4x and WGM41..0 :
        // PWM4x=1, WGM41_40=00 --> Fast PWM
        TCCR4C |= (1<<PWM4D);   // PWM4D enable, Set PWM4D=1
        TCCR4E |= (1<<ENHC4);   // Set Enhanced PWM Mode = TCCR4E.6
        // Set PortD Operation Mode with COM4D1..0 : COM4D1..0=11 
        // -> only Pin OC4D connected, inverted
      ...
      #endif
    #elif defined(EXT_MOTOR_64KHZ) || defined(EXT_MOTOR_32KHZ) || defined(EXT_MOTOR_16KHZ) || defined(EXT_MOTOR_8KHZ)
      TCCR1A = (1<<WGM11);
  ...
  #endif
  #if defined(MOTOWII_REVB)
    if (!(PINE & (1<<2)))              // check for jumper installed
      calibrateESCs();
  #endif
...
#if defined(ONESHOT)
  ISR(TIMER4_OVF_vect) {
    TCCR4B = 0;     // Stop Timer4 -> Prescaler=0
    TCNT4 = 0;      // Force output to clear
  }
#endif
